...
const LEGACY_FOLLOW_UPS = [
  ...
  },
  {
    ...
    daysToNext: -2,
    priority: "high" as const,
    contactStatus: "unreachable" as const,
...
interface CustomerFlowContextValue {
  ...
  createFittingFromFlow: (
    data: Partial<WorkflowFittingRecord> & { fittingId?: string }
  ) => WorkflowFittingRecord | null;
...
export function CustomerFlowProvider({ children }: { children: ReactNode }) {
  const { customers, listCustomers, createFitting, getLatestComparison } = useArchive();

  const { createRecord, updateRecord, submitForReview, getFilteredRecords } = useWorkflow();
  ...
  const createFittingAndWorkflow = useCallback(
    async (
      fittingData: Partial<FittingRecord>,
      workflowData?: Partial<WorkflowFittingRecord>
    ): Promise<{ fitting: FittingRecord; workflow: WorkflowFittingRecord }> => {
      ...
      const hearingAidModel =
        fitting.hearingAid?.left?.model || fitting.hearingAid?.right?.model || "";
      ...
      const workflowStage = (fitting.stage === "随访" ? "复诊" : fitting.stage) as
        | "初配"
        | "复调"
        | "复诊"
        | undefined;
      const workflowRecordData: Partial<WorkflowFittingRecord> = {
        ...
        speechRecognitionRate:
          typeof speechRecognitionRate === "number" ? speechRecognitionRate : 0,
  ...
  const createFittingFromFlow = useCallback(
    (
      data: Partial<WorkflowFittingRecord> & { fittingId?: string }
    ): WorkflowFittingRecord | null => {
      ...
      return (
        allRecords.find(
          (r) =>
            r.customerId === aggregate.activeCustomerProfile!.id ||
            r.customerId === aggregate.activeCustomerProfile!.customerNo
        ) || null
      );
    },
    [
      aggregate.activeCustomerProfile,
      aggregate.activeLatestWorkflowRecord,
      createRecord,
      updateRecord,
      getFilteredRecords
    ]
