...
export function computeFeedbackStatus(initial: string, followUp: string): { status: ComparisonStatus; label: string } {
...
export function computeGainStatus(initial: string, followUp: string): { status: ComparisonStatus; label: string } {
  if (!initial || !followUp) return { status: "stable", label: "待评估" };
  const initLower = initial.toLowerCase();
  const followLower = followUp.toLowerCase();
...
export function generateComparisonResults(data: ComparisonData): ComparisonResultItem[] {
  const speechResult = computeSpeechStatus(data.initial.speechRecognitionRate, data.followUp.speechRecognitionRate);
  const feedbackResult = computeFeedbackStatus(data.initial.feedbackWhistle, data.followUp.feedbackWhistle);
  const gainResult = computeGainStatus(data.initial.gainAdjustment, data.followUp.gainAdjustment);

  return [
    {
      label: "言语识别率",
      initialValue: data.initial.speechRecognitionRate !== null ? `${data.initial.speechRecognitionRate}%` : "未填写",
      followUpValue: data.followUp.speechRecognitionRate !== null ? `${data.followUp.speechRecognitionRate}%` : "未填写",
